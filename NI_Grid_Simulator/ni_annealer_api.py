...
def _normalise_groups(value: Any) -> tuple[int, ...]:
    ...
    invalid = [g for g in groups if g not in (1, 2, 3, 4, 5)]
    if invalid:
...
def _build_node_table(renewables: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, a in renewables.reset_index(drop=True).iterrows():
        groups = tuple(g for g in (1, 2, 3, 4, 5) if bool(masks[g][i]))
        rows.append({
            "node_id": int(i),
...
def _masks_from_nodes(nodes: pd.DataFrame, n_nodes: int) -> dict[int, np.ndarray]:
    ...
    masks = {g: np.zeros(n_nodes, dtype=bool) for g in (1, 2, 3, 4, 5)}
    for row in nodes[["node_id", "groups"]].itertuples(index=False):
        node_id = int(row.node_id)
        for g in _normalise_groups(row.groups):
            masks[g][node_id] = True
    return masks
...
def make_emulator(
    ...
    baseline_repr_secure = baseline.physical.run_table["represented_security_pass"].astype(bool).to_numpy()
    baseline_n1_secure = baseline.physical.run_table["all_line_n1_pass"].astype(bool).to_numpy()
    ...
    def dispatch_down(nodes: pd.DataFrame) -> float:
        ...
        candidate_repr = result.run_table["represented_security_pass"].astype(bool).to_numpy()
        candidate_n1 = result.run_table["all_line_n1_pass"].astype(bool).to_numpy()
        ...
        dispatch_down.last_represented_security_pass_pct = float(result.metrics["represented_security_pass_pct"])
        dispatch_down.last_all_line_n1_pass_pct = float(result.metrics["all_line_n1_pass_pct"])
        dispatch_down.last_new_represented_security_failures = int(np.sum(baseline_repr_secure & ~candidate_repr))
        dispatch_down.last_new_n1_security_failures = int(np.sum(baseline_n1_secure & ~candidate_n1))
