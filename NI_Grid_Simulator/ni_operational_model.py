"""Northern Ireland operational baseline model.

Builds frozen operating scenarios and evaluates the published/current renewable
constraint-group policy.  This module deliberately contains no search, annealing,
or group optimisation logic; it is the physics/operations layer intended for
reuse by a separate optimiser.
"""
...
class OperationalConfig:
    ...
    security_redispatch_backstop: bool = True
    security_backstop_max_actions: int = 24
...
def evaluate_policy(
    ...
    tbus_idx = np.array([model.bus_index[str(b)] for b in therm["mapped_bus"]], dtype=int)
    H_thermal = Hfull[line_idx[:, None], tbus_idx[None, :]].T  # thermal asset x line
    thermal_min = tcap * float(cfg.min_stable_output_pct) / 100.0
    ...
    security_backstop_dd_total = 0.0
    security_redispatch_mw_total = 0.0
    ...
    for i in range(runs):
        ...
        thermal_i = scenarios.initial_thermal_dispatch_mw[i].copy()
        seq: list[int] = []
        curts: list[float] = []
        used: set[int] = set()
        security_backstop_dd = 0.0
        security_redispatch_mw = 0.0
        ...
        # General operator-security backstop.  The five WDT groups do not encode every
        # possible N-1 thermal action used by a real control room.  First try
        # zero-net-energy thermal redispatch; if that cannot relieve the worst
        # credible N-1 overload, use electrically targeted renewable reduction
        # as a last-resort security proxy.  The latter is reported separately
        # from named group dispatch-down so downstream optimisers can audit it.
        if bool(getattr(cfg, "security_redispatch_backstop", True)) and cfg.include_n1_diagnostic:
            for _ in range(int(getattr(cfg, "security_backstop_max_actions", 24))):
                if planned_k >= 0:
                    base_ratio = np.abs(f) / limits
                    mon = int(np.argmax(base_ratio)); out = None; ratio = float(base_ratio[mon])
                else:
                ...
                # 1) Thermal redispatch: move MW from one online unit to another.
                thermal_candidates = []
                for down in range(len(thermal_i)):
                    down_room = max(0.0, float(thermal_i[down] - (thermal_min[down] if thermal_i[down] > 1e-8 else 0.0)))
                    if down_room <= 1e-8:
                        continue
                    for up in range(len(thermal_i)):
                        if up == down:
                            continue
                        up_room = max(0.0, float(tcap[up] - thermal_i[up]))
                        cap_pair = min(down_room, up_room)
                        if cap_pair <= 1e-8:
                            continue
                        sens_line = H_thermal[up] - H_thermal[down]
                        ss = _state_sensitivity(sens_line, lodf, mon, out)
                        req = _required_reduction(sf, limits[mon], ss)
                        if np.isfinite(req) and req > 1e-9:
                            thermal_candidates.append((float(req), -cap_pair, down, up, sens_line, cap_pair))
                if thermal_candidates:
                    thermal_candidates.sort(key=lambda x: (x[0], x[1]))
                    req, _negcap, down, up, sens_line, cap_pair = thermal_candidates[0]
                    delta = min(req * 1.000001 + 1e-7, cap_pair)
                    if delta > 1e-8:
                        f += delta * sens_line
                        ffull[line_idx] = f
                        thermal_i[down] -= delta
                        thermal_i[up] += delta
                        security_redispatch_mw += delta
                        continue

                # 2) Last-resort targeted renewable security reduction, balanced
                # at the model balancing thermal bus. Choose the single most
                # efficient available renewable asset for the active overload.
                ren_candidates = []
                for j in range(len(dispatch)):
                    available = float(dispatch[j])
                    if available <= 1e-8:
                        continue
                    sens_line = -transfer_asset[j]
                    ss = _state_sensitivity(sens_line, lodf, mon, out)
                    req = _required_reduction(sf, limits[mon], ss)
                    if np.isfinite(req) and req > 1e-9:
                        ren_candidates.append((float(req), -available, j, sens_line, available))
                if not ren_candidates:
                    break
                ren_candidates.sort(key=lambda x: (x[0], x[1]))
                req, _negavail, j, sens_line, available = ren_candidates[0]
                delta = min(req * 1.000001 + 1e-7, available)
                if delta <= 1e-8:
                    break
                f += delta * sens_line
                ffull[line_idx] = f
        ...
        group_network_dd = float(sum(curts))
        network_dd = group_network_dd + float(security_backstop_dd)
        network_dd_total += network_dd
        security_backstop_dd_total += float(security_backstop_dd)
        security_redispatch_mw_total += float(security_redispatch_mw)
        ...
        row = {
            ...
            "network_group_dispatch_down_mw": group_network_dd,
            "security_backstop_dispatch_down_mw": float(security_backstop_dd),
            "security_thermal_redispatch_mw": float(security_redispatch_mw),
            "total_network_dispatch_down_mw": network_dd,
    ...
    named_group_dd_total = float(sum(group_dd.values()))
    metrics = {
        "runs": runs,
        ...
        "mean_network_group_dispatch_down_mw": named_group_dd_total / runs,
        "mean_total_network_dispatch_down_mw": network_dd_total / runs,
        "mean_security_backstop_dispatch_down_mw": security_backstop_dd_total / runs,
        "mean_security_thermal_redispatch_mw": security_redispatch_mw_total / runs,
        ...
        "network_group_dispatch_down_mw_total": named_group_dd_total,
        "security_backstop_dispatch_down_mw_total": security_backstop_dd_total,
        "total_network_dispatch_down_mw_total": network_dd_total,
        "security_thermal_redispatch_mw_total": security_redispatch_mw_total,
