...
PROJECT_DIR = VALIDATOR_DIR.parent

# Existing functions in this file still refer to ROOT
ROOT = PROJECT_DIR
...
# IMPORTANT: this must still be here
...
DEFAULT_NODES_32 = (
    PROJECT_DIR
    / "outputs"
    / "annealer_nodes_32_counties_sv2024_wdt_exclusive.csv"
)
...
def _synthetic_renewable_potential(nodes: pd.DataFrame, n_snap: int, seed: int, wind_scale: float = 1.0, solar_scale: float = 1.0) -> np.ndarray:
    ...
    tech_scale = np.ones(n_node, dtype=float)
    for j, t in enumerate(tech):
        tech_scale[j] = float(solar_scale) if ("solar" in t or "pv" in t) else float(wind_scale)
    availability = np.clip(availability * tech_scale[None, :], 0.0, 1.0)
...
def _build_frozen_cases(
    ...
    potential = _synthetic_renewable_potential(nodes, n_snap, seed, wind_scale=wind_scale, solar_scale=solar_scale)
    ...
    for load_id in network.load_profile.columns:
        ...
        if b in model.bus_index:
            demand[:, model.bus_index[b]] += network.load_profile[str(load_id)].to_numpy(float) * float(demand_scale)
...
def make_emulator(
    ...
    # Compatibility with calls written for ni_annealer_api. The historical
    # calibration controls remain unused, while demand_scale is active here for
    ...
    """
    del target_dispatch_down_pct, planned_outage_exposure_pct
    demand_scale = 1.0 if demand_scale is None else float(demand_scale)
    if demand_scale <= 0 or float(wind_scale) <= 0 or float(solar_scale) <= 0:
        raise ValueError("demand_scale, wind_scale and solar_scale must be positive")
    ...
    def prepare_grouping_evaluator(_nodes: pd.DataFrame | None = None):
        """Return a fast integer-group-array evaluator for large annealing studies."""
        def fast_energy(group_ids) -> float:
            arr = np.asarray(group_ids, dtype=int)
            if arr.ndim != 1 or len(arr) != len(template):
                raise ValueError("group_ids must be a 1-D array with one entry per renewable node")
            pct, secure, _worst, _network_dd = evaluate_assignment(arr)
            new_fail = baseline_secure & ~secure
            if bool(security_guard) and np.any(new_fail & (weights > 0)):
                return float("inf")
            return float(pct)
    ...
    dispatch_down.include_n1 = bool(include_n1)
    dispatch_down.thermal_scale = float(thermal_scale)
    dispatch_down.demand_scale = float(demand_scale)
    dispatch_down.wind_scale = float(wind_scale)
    dispatch_down.solar_scale = float(solar_scale)
    dispatch_down.security_screen_threshold_pct = float(security_screen_threshold_pct)
    dispatch_down.max_security_states = int(max_security_states)
